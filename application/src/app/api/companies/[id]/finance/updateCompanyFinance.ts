...
export const updateCompanyFinance = async (
  request: NextRequest,
  user: { id: string },
  ...
  try {
    ...
    if (!existing) {
      return NextResponse.json(
        { error: 'Finance record not found for company' },
    ...
    const finance = await result.dbClient.companyFinance.update(id, parsed.data);

    return NextResponse.json({ finance }, { status: HTTP_STATUS.OK });
  } catch (error) {
    console.error('Error updating company finance record:', error);
    return NextResponse.json(
      { error: 'Failed to update company finance record' },
